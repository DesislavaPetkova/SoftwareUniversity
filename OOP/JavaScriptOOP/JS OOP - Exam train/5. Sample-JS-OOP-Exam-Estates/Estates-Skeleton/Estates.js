function processEstatesAgencyCommands(commands) {
    ...
    var Estate = (function() {
        ...
        function Estate(name, area, location, isFurnitured) {
    		...
    		throw new Error("Cannot initialize abstract method");
    		}
        	this.setName(name);
        	this.setArea(area);
    ...
    var BuildingEstate = (function() {   	
    	function BuildingEstate(name, area, location, isFurnitured, rooms, hasElevator) {
        	...
        	this.setRooms(rooms);
        	this.setHasElevetor(hasElevator);
        }     
        BuildingEstate.extends(Estate);


        BuildingEstate.prototype.getIsFurnitured = function() {
        	return this._isFurnitured;
        };

        BuildingEstate.prototype.setIsFurnitured = function(isFurnitured) {
        	this._isFurnitured = isFurnitured;
        };
        ...
        BuildingEstate.prototype.setHasElevetor = function(hasElevator) {
    ...
    var EstatesEngine = (function() {
        ...
        function executeFindRentsByPriceCommand(minPrice, maxPrice) {
        	...
        	if(isNaN(minPrice) || isNaN(maxPrice)){
        		throw new Error('Price should be positive numbers');
        	}
            ...
            var selectedOffers = _offers.filter(function(offer) {
            	...
            	if(result == 0){
                	var result = a.getEstate().getName().localeCompare(b.getEstate().getName());
            	}
            	return result
            });
            return formatQueryResults(selectedOffers);
	...
	// var gouse = new Apartment("name", 123, 'sofia', true, 2, true);
	// var offer = new RentOffer('', -242242);
